...
# Encoding for the 5 RCC-5 relations ≥ ≤ ≳ ≲ ≂
# and their disjunctions
...
# RCC-5 relasionship WITHIN hierarchy
...
def def_rcc5_symbol(n, name, exname=None):
  ...
  rcc5_symbols[n] = name
  rcc5_relasionships[name] = n
  rcc5_relasionships[exname] = n
  return n
...
rcc5_relasionships['*'] = OVERLAP # for concise composition table
...
def rcc5_relasionship(name):
  if name.startswith('{'):
    ship = 0
    for name in name[1:-1].split(' '):
      ship |= rcc5_relasionship(name)
    return ship
  return rcc5_relasionships[name]

# The table only goes from pairs of single RCC-5 relasionships to a
# the set of the possible resulting RCC-5 relasionships.  If the
# inputs are themselves sets, we just iterate in order to get all
# possible results for all possible inputs.

def compose_relasionships(r, s):
  ship = 0
...
# -----------------------------------------------------------------------------
# Composing RCC5 relasionships

# This table was automatically generated by the code that follows it.
# Alternatively, see https://doi.org/10.1007/11551263_7
...
rcc5_composition_table = \
  {(rcc5_relasionship(r),rcc5_relasionship(s)): rcc5_relasionship(symbolic_rcc5_composition_table[(r,s)])
...
if __name__ == '__main__':
  #print(rcc5_symbol(compose_relasionships(LT, LT)))
