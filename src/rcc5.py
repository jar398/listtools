...
# RCC-5 relation WITHIN hierarchy
...
rcc5_relations['*'] = OVERLAP # for concise composition table
...
def rcc5_relation(name):
  if name.startswith('{'):
    ship = 0
    for name in name[1:-1].split(' '):
      ship |= rcc5_relation(name)
    return ship
  return rcc5_relations[name]

# The table only goes from pairs of single RCC-5 relations to a
# the set of the possible resulting RCC-5 relations.  If the
...
rcc5_composition_table = \
  {(rcc5_relation(r),rcc5_relation(s)): rcc5_relation(symbolic_rcc5_composition_table[(r,s)])
...
if __name__ == '__main__':
  #print(rcc5_symbol(compose_relations(LT, LT)))
