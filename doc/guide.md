...
<strong>Warning:</strong> The code is in transition from one tree
comparison method to another.  The older method worked in 2021; the
newer method is just now working as of October 2022. You will see
redundancy in functionality, since the older code has just been left
in place rather than removed or integrated with the new code.


## Installation
<a name="installation"/>

The tools require no particular installation step.  They are generally
invoked out of a clone of this `git` repository using `make` or from
the shell using `python3`.
...
`Makefile` assumes that `gnparser` can be located via your shell's
`PATH`.  It is easy to remove this dependency if desired, with a
degradation in effectiveness, by modifying the appropriate
`Makefile` rules to elide the `extract_names` and `use_gnparse` steps.
...
for some modest validation and DwC-specific cleaning.
    ...
    src/start.py --pk taxonID --input foo-raw.tsv >foo.csv
...
unprocessed Darwin Core file.  This is assumed to be in CSV
 ...
 * `year` is added
 * `type` is added - that's as in 'type specimen'.  
   This is a special string used for matching, and has 
   ...
   to be associated with the name's type specimen or type series, and not any
...
feature is a big help in matching names when the genus assignment
changes or taxa or promoted or demoted between species and subspecies.
